...
async function getFinancialAdviceOnSavingGoal(prompt) {
  try {
    ...
    const result = await model.generateContent(
      `I made a plan to achive my goal I want to achive in here is the data of it ${JSON.stringify(
        prompt
      )}, cuurency unit:- ruppes(INR) time_frame unit:- days, is theplan feasible and recommend how to improve the plan give a specific. Limit the response to a few sentences. `
    );
