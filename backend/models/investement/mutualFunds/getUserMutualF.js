...
const getUserMutualFunds = async (userId) => {
  ...
  const investmentDetails = await pool.query(
    `SELECT 
  ...
  if (investmentDetails.rows.length > 0) {
    totalInvestedAmount = parseFloat(
      investmentDetails.rows[0].total_invested_amount
    );
    totalCurrentValue = parseFloat(
      investmentDetails.rows[0].total_current_value
    );
  }
  redis.set(
    `userMutualFunds:${userId}`,
    JSON.stringify({
